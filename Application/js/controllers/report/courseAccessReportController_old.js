managementApp.controller('courseAccessReportController',['$rootScope', '$scope', '$location', '$http', 'config', function($rootScope, $scope, $location, $http, config) {
	...
	$scope.submitDownload = function(report) {
		...
		var url = config.baseURL + 'reports/courseAccessReportCSV.php?' + queryString;
		window.open(url);
	}
	...
	$scope.submitForm = function(report){
		...
		$http({
			...
			url : config.baseURL + config.report.courseAccess
			}).then(function mySuccess(response) {
