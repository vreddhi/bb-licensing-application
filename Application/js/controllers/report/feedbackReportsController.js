managementApp.controller('feedbackReportsController',['$rootScope', '$scope', '$location', '$http', 'config', function($rootScope, $scope, $location, $http, config) {
	...
	$scope.submitDownload = function(report) {
		...
		var url = config.baseURL + 'reports/feedbackReportCSV.php?' + queryString;
		window.open(url);
	}
	...
	$scope.submitForm = function(report){
		...
		$http({
			...
			url : config.baseURL + config.report.feedback
			}).then(function mySuccess(response) {
